...
async def check_in_worker(worker_id):
    ...
    # Timezone ni olib tashlab saqlaymiz (Postgres uchun)
    naive_now = now.replace(tzinfo=None)
    
    exists = await conn.fetchrow("SELECT id FROM attendance WHERE worker_id=$1 AND date=$2", worker_id, today)
    if exists:
        await conn.execute("UPDATE attendance SET start_time=$1, status='Keldi' WHERE id=$2", naive_now, exists['id'])
    else:
        await conn.execute("INSERT INTO attendance (worker_id, date, start_time, hours, status) VALUES ($1, $2, $3, 0, 'Keldi')", worker_id, today, naive_now)
    await conn.close()
...
async def check_out_worker(worker_id):
    ...
    # Bazadan olingan vaqtni TZ_UZ ga o'tkazamiz
    start_time = row['start_time'] # Naive
    duration = (naive_now - start_time).total_seconds() / 3600
    hours = round(duration, 1)
    
    await conn.execute("UPDATE attendance SET end_time=$1, hours=$2 WHERE id=$3", naive_now, hours, row['id'])
    await conn.close()
    return True, hours
...
async def get_worker_stats(telegram_id):
    conn = await get_db()
    month = datetime.now(TZ_UZ).strftime("%Y-%m")
...
async def get_month_attendance(year, month): # Status uchun
    conn = await get_db()
    rows = await conn.fetch("SELECT worker_id, hours FROM attendance WHERE TO_CHAR(date, 'YYYY-MM') = $1", f"{year}-{month:02d}")
    await conn.close()
    return rows

async def get_month_advances(year, month): # Status uchun
    conn = await get_db()
